...
describe('generatePassphrase', () => {
  describe('Basic Functionality', () => {
    ...
    it('should generate a passphrase with maximum word count of 10', () => {
      const passphrase = generatePassphrase(PASSPHRASE_CONSTRAINTS.MAX_WORDS, '-')
      const words = passphrase.split('-')
      expect(words).toHaveLength(PASSPHRASE_CONSTRAINTS.MAX_WORDS)
    })

    it('should use hyphen separator by default', () => {
      const passphrase = generatePassphrase(4, SEPARATORS.hyphen)
  ...
  describe('Separator Options', () => {
    it('should use space separator', () => {
      const passphrase = generatePassphrase(4, SEPARATORS.space)
      expect(passphrase).toContain(' ')
      expect(passphrase).not.toContain('-')
    })
    ...
    it('should use underscore separator', () => {
      const passphrase = generatePassphrase(4, SEPARATORS.underscore)
      expect(passphrase).toContain('_')
      expect(passphrase).not.toContain('-')
      expect(passphrase).not.toContain(' ')
    })

    it('should use period separator', () => {
      const passphrase = generatePassphrase(4, SEPARATORS.period)
      expect(passphrase).toContain('.')
      expect(passphrase).not.toContain('-')
      expect(passphrase).not.toContain(' ')
    })
  ...
  describe('Word Properties', () => {
    it('should generate passphrases with lowercase words', () => {
      const passphrase = generatePassphrase(4, '-')
      const words = passphrase.split('-')
      words.forEach((word) => {
    ...
    it('should generate passphrases with reasonable word lengths', () => {
    ...
    it('should not include special characters in words', () => {
      const passphrase = generatePassphrase(4, '-')
      const words = passphrase.split('-')
      words.forEach((word) => {
        expect(word).toMatch(/^[a-z]+$/)
  ...
  describe('Edge Cases and Boundary Conditions', () => {
    it('should handle minimum word count (4)', () => {
      const passphrase = generatePassphrase(PASSPHRASE_CONSTRAINTS.MIN_WORDS, '-')
      const words = passphrase.split('-')
      expect(words).toHaveLength(PASSPHRASE_CONSTRAINTS.MIN_WORDS)
    })

    it('should handle maximum word count (10)', () => {
      const passphrase = generatePassphrase(PASSPHRASE_CONSTRAINTS.MAX_WORDS, '-')
      const words = passphrase.split('-')
      expect(words).toHaveLength(PASSPHRASE_CONSTRAINTS.MAX_WORDS)
    })
    ...
    it('should generate different passphrases on multiple calls', () => {
      const passphrase1 = generatePassphrase(4, '-')
      const passphrase2 = generatePassphrase(4, '-')
    ...
    it('should handle empty separator (no separator)', () => {
      const passphrase = generatePassphrase(4, '')
      expect(passphrase).not.toContain('-')
      expect(passphrase).not.toContain(' ')
      expect(passphrase).not.toContain('_')
  ...
  describe('Security and Randomness', () => {
    ...
    it('should generate cryptographically secure passphrases', () => {
      const passphrases = new Set()
      for (let i = 0; i < 100; i++) {
        passphrases.add(generatePassphrase(4, '-'))
      }
  ...
  describe('Performance', () => {
    it('should generate passphrase within performance target', () => {
      const start = performance.now()
      generatePassphrase(4, '-')
      const duration = performance.now() - start
      expect(duration).toBeLessThan(PERFORMANCE_TARGETS.GENERATION_TIME_MS)
    ...
    it('should generate multiple passphrases efficiently', () => {
      const start = performance.now()
      for (let i = 0; i < 100; i++) {
        generatePassphrase(4, '-')
      }
